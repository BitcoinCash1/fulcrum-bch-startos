...
- **Knuth is wired like BCHN.** `knuth-bch-startos/startos/utils` exports `networkPorts` and `rpcInterfaceId` (`'rpc'`), and its RPC port moves per chain. Its JSON-RPC is served by the `rpc-compat` sidecar daemon, so that is the health check the dependency requires; the node task enables JSON-RPC and the full database mode.
